...
class ProfilePresenter {
  ...
  Future<void> uploadImage(File image) async {
    ...
    DataSnapshot snapshot =
        await _databaseRef.child("users").child(currentUser.uid).once();
    String pastUrl = await _storageRef
        .child("profileImages/" + snapshot.value['image'])
        .getDownloadURL();
    StorageReference ref = await FirebaseStorage().getReferenceFromUrl(pastUrl);
    ref.delete();

    StorageReference firebaseStorageRef =
        _storageRef.child("profileImages/").child(currentUser.uid);
